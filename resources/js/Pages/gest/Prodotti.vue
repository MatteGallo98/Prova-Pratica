<template>
  ...
  <tr v-for="(product, key) in data.products.data" :key="key">
    ...
    <td>
      <Link :href="route('product.edit', { id: product.id })">Modifica</Link>
    </td>
    <td>
      <input
        type="submit"
  ...
  methods: {
    cancella(id) {
      if (confirm("Vuoi veramente cancellarlo?")) {
        this.$inertia.post(route('product.destroy', { id: id, perPage: this.data.perPage }));
      }
    },
    format_date(value) {
      if (value) {
