<template>
    ...
    <tr v-for="(order, key) in data.orders.data" :key="key">
        <td>{{order.email}}</td>
        <td>
                <span v-for="(product, key) in order.products" :key="key"> {{product.name+ " "}}{{product.PS==0 ? "(P)" : "(S)"}}</span><br/> 
        </td>
        <td>{{order.status}}</td>
        ...
        <td>
                    <input type="submit" value="Cancella" class="buttonSubmit" @click="cancella(order.id)">
        </td>
    </tr>
</template>
